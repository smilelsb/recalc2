...
const RATIO_FINDER_PATH = '/zh/ratio-finder';
const RATIO_FINDER_TITLE = 'FRC 与 FTC 传动比查找器 | ReCalc';
const RATIO_FINDER_NAME = '传动比查找器';
const RATIO_FINDER_DESCRIPTION =
  '为 FRC 和 FTC 机器人查找合适的齿轮、同步带和链条传动比，在链轮、同步带轮和齿轮组合中搜索目标减速比。';
...
export default function ChineseRatioFinder() {
    ...
    <div data-testid="ratio-finder-page" data-calculating={String(loading)}>
      ...
      <div className="flex flex-row flex-wrap gap-6 px-1">
          ...
          <section className="flex flex-col rounded-lg border">
            ...
            <div className="flex flex-col gap-3 p-4">
              ...
              <p className="text-xs text-muted-foreground">
                限制每一级可以使用的动力传动类型。查找器会先搜索一级或两级方案，只有在其他方案无法达到目标时才加入第
                3 级，因此第 3 级筛选只在这种情况下生效。
              </p>
              <div className="grid grid-cols-1 gap-3 md:grid-cols-3">
                <StageFamilySelect
        ...
        <div className="flex min-w-75 flex-1 flex-col gap-x-4 gap-y-2">
          <Divider>{loading ? <Spinner /> : <>{count} 个方案</>}</Divider>
          {autoExpandedToThreeStage && (
            ...
            >
              当前筛选下没有一级或两级 gearbox
              能达到此传动比，正在显示三级方案。
            </div>
          )}
          {solutions.length === 0 ? (
            <div className="text-muted-foreground">未找到方案</div>
          ) : (
                                ...
                                {stage.from.skus.map((sku, skuIndex) => (
                                  ...
                                  >
                                    ...
                                    <span className="text-muted-foreground">
                                      ...
                                      {skuFamilyLabel(sku.family)} ({sku.bore})
                                    </span>
                                  </Link>
                                ))}
                              </div>
